...
public class AppConfigurationService : IAppConfigurationService
{
    ...
    public async Task<BaseResponse<AppConfigurationOutput>> Update(AppConfigurationInput input)
    {
        ...
        entity.SeriesCommitmentsEnabled = input.SeriesCommitmentsEnabled;
        entity.SeriesDecisionHours = Math.Clamp(input.SeriesDecisionHours, 0, SeriesRules.MaxDecisionHours);
        entity.SeriesSkipNoticeHours = Math.Clamp(input.SeriesSkipNoticeHours, 0, SeriesRules.MaxSkipNoticeHours);
        entity.SeriesFreeSkipsPerWindow = Math.Clamp(input.SeriesFreeSkipsPerWindow, 0, SeriesRules.MaxFreeSkips);
        entity.SeriesEndNoticeDays = Math.Clamp(input.SeriesEndNoticeDays, 0, SeriesRules.MaxEndNoticeDays);
        entity.SeriesSummaryDay = Math.Clamp(input.SeriesSummaryDay, 0, 6);
