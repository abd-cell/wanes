...
/// Hailing a ride — the screen the rider watches while drivers are pinged.
///
/// It reaches for SSE on open, which fails closed in a test binding, so what
/// is asserted here is the still-searching state: nobody has accepted yet.
void main() {
  ...
  group('with the rider coordinates', () {
    Future<void> pump(WidgetTester tester) async {
      await tester.pumpWidget(host(const SearchingScreen(
        rideRequestId: 12,
        driversNotified: 2,
        originLat: 31.9539,
        originLng: 35.9106,
    ...
    testWidgets('draws a real map on the hail, not the illustration',
        (tester) async {
      await pump(tester);
    ...
    testWidgets('has no car until a driver accepts', (tester) async {
      await pump(tester);
      final map = tester.widget<LiveTripMap>(find.byType(LiveTripMap));

      // Nobody has accepted, so there is no driver to draw and no position to
      // draw them at.
      expect(map.progress.marker, isNull);
      expect(map.driverAt, isNull);
      // The ride is only a request so far — the route reads as not-yet-happening.
      expect(map.progress.ahead, MapLegStyle.faint);
    });

    testWidgets('pings out from the rider pin while it searches', (tester) async {
      await pump(tester);
      await tester.pump();
  ...
  group('without coordinates', () {
    testWidgets('falls back to the prototype illustration', (tester) async {
      ...
      await tester.pumpWidget(host(const SearchingScreen(rideRequestId: 12)));
      await tester.pump();
